...
## Level 0
def ApplyPrePostSilence(SoundPulseFiltered, Rate, SoundPulseDur, 
                        SoundPrePauseDur=0, SoundPostPauseDur=0, SoundPulseNo=1):
    
    SoundPrePause = np.zeros((round(Rate * SoundPrePauseDur),1), dtype=np.float32)
    SoundPostPause = np.zeros((round(Rate * SoundPostPauseDur),1), dtype=np.float32)
    
    SoundFiltered = {}
    for F in SoundPulseFiltered.keys():
        TotalDur = (SoundPrePauseDur + SoundPulseDur + SoundPostPauseDur)*Rate
        SoundFiltered[F] = np.zeros((int(TotalDur*SoundPulseNo),1))
        
        for P in range(1,SoundPulseNo):
            Start = int(TotalDur*(P-1))
            End = int(TotalDur*P)
            SoundFiltered[F][Start:End] = np.concatenate([SoundPrePause, 
                                                np.split(SoundPulseFiltered[F], SoundPulseNo)[P-1],
                                                SoundPostPause])
    
    return(SoundFiltered)


def ApplySoundAmpF(SoundPulseFiltered, SoundAmpF, SBOutAmpF):
    print('Applying amplification factors...')
    SoundUnit = {}
    
    for FKey in SoundPulseFiltered.keys():
        SoundUnit[FKey] = {}
        
        for AmpF in SoundAmpF[FKey]:       
            if AmpF > 1/SBOutAmpF:
                print(AmpF,  'AmpF out of range. Decreasing to', 1/SBOutAmpF, '.')
                AmpF = 1/SBOutAmpF
            
            AKey = str(AmpF)
            SoundUnit[FKey][AKey] = (SoundPulseFiltered[FKey] * AmpF) * SBOutAmpF
...
def Noise(Rate, SoundPulseDur, SoundPulseNo=1):
    print('Generating noise...')
    Noise = np.random.uniform(-1, 1, size=(round(Rate*SoundPulseDur*SoundPulseNo),1))
    Noise[-1] = 0
...
def SoundStim(Rate, SoundPulseDur, SoundAmpF, NoiseFrequency, TTLAmpF, 
              ...
              SoundPulseNo=1, TTLs=True, Map=[1,2]):
    ...
    SoundPulse = Noise(Rate, SoundPulseDur, SoundPulseNo)
    print('   ', end='')
    SoundPulseFiltered = BandpassFilterSound(SoundPulse, Rate, NoiseFrequency)
    print('   ', end='')
    SoundFiltered = ApplyPrePostSilence(SoundPulseFiltered, Rate, SoundPulseDur, 
                        SoundPrePauseDur, SoundPostPauseDur, SoundPulseNo)
    
    SoundUnit = ApplySoundAmpF(SoundFiltered, SoundAmpF, SBOutAmpF)
    TotalDur = (SoundPrePauseDur + SoundPulseDur + SoundPostPauseDur)*Rate*SoundPulseNo
